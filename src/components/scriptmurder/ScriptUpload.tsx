...
export default function ScriptUpload() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            formData.append('playerCount', playerCount.toString());

            // Upload character scripts
            characterScripts.forEach((item, index) => {
                formData.append(`characterScript_${index}`, item.file);
            });

            // Upload DM handbook
            if (dmHandbook) {
                formData.append('dmHandbook', dmHandbook.file);
            }
            ...
            if (response.ok) {
                toast({
                    title: '上传成功',
                    description: `剧本《${scriptName}》已成功上传并解析`,
                });
            ...
            <div className="space-y-6">
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        {/* Uploaded Character Scripts List */}
                                    ...
                                    {characterScripts.map((item, index) => (
                                        ...
                                        >
                                            ...
                                            <Button
                                                variant="ghost"
                                                size="sm"
                                                onClick={() => removeCharacterScript(index)}
                        ...
                        {characterScripts.length !== playerCount && characterScripts.length > 0 && (
                            <Alert>
                                ...
                                <AlertDescription>
                                    角色剧本数量（{characterScripts.length}）与玩家人数（{playerCount}）不匹配
                                </AlertDescription>
                            </Alert>
                        )}
                    </CardContent>
                    ...
                    <CardContent className="space-y-4">
                        {!dmHandbook ? (
                            ...
                            <div className="flex items-center gap-3 p-4 border rounded-lg bg-muted/30">
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-medium truncate">
                                        {dmHandbook.file.name}
                                    </p>
                                    <p className="text-xs text-muted-foreground">
                                        {(dmHandbook.file.size / 1024 / 1024).toFixed(2)} MB
                                    </p>
                                </div>
                ...
                <div className="flex gap-4">
                    <Button
                        ...
                        disabled={isUploading || !scriptName || characterScripts.length === 0 || !dmHandbook}
                        className="flex-1"
                    >
                        {isUploading ? (
                            <>
