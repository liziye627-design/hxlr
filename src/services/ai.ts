...
function buildOfficialPersonaPrompt(companion: AICompanion): string {
...
async function chatWithGemini(messages: AIMessage[], companion?: AICompanion): Promise<string> {
  ...
  const toGeminiContents = (items: AIMessage[]) =>
    items.map((message) => ({
...
export const aiService = {
  async chat(messages: ChatMessage[], companion?: AICompanion): Promise<string> {
    const formattedMessages: AIMessage[] = messages.map((message) => ({
      role: message.role,
